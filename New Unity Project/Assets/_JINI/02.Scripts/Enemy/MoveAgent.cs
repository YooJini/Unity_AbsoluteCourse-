...
public class MoveAgent : MonoBehaviour
{
    ...
    private readonly float patrolSpeed = 1.5f; //런타임에서 변수값을 변경할 필요가 없는 변수는 readonly 또는 const 키워드 사용 => 가비지 컬렉터에 쌓이지 않도록 함
    ...
    //주인공을 추적할 때 이동시키는 함수
    void TraceTarget(Vector3 traceTarget)
    {
    ...
    void Start()
    {
        ...
        //하이러키 뷰의 WayPointGroup 게임 오브젝트 추출
        //GameObject.Find~ 유형의 함수는 처리속도가 느림
        //가급적 Update 함수 내에서는 사용하지 말것 
   ...
   public void Stop()
    ...
    void Update()
    {
        ...
        if(!agent.isStopped)
        {
            ...
            //desiredVelocity: 다음 목적지로 향하는 속도를 의미함, LookRotation(속도) -> 인자로 전달된 벡터가 바라보는 방향으로의 회전 각도를 쿼터니언 타입으로 반환
            Quaternion rot = Quaternion.LookRotation(agent.desiredVelocity); 
            //보간 함수를 사용해 점진적으로 회전시킴(부드럽게)
            //Quaternion.Slerp(Quaternion a, Quaternion b, float t) -> a각도에서 b각도 사이를 시간 t에 따라 점진적으로 반환하는 함수
        ...
        //적캐릭터가 처음엔 정지 상태이기 때문에 remainingDistance 변수값은 항상 0을 반환함
        //따라서 캐릭터가 이동중이라는 조건도 함께 판단해야 함
